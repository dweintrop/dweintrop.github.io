...
/**
 * Return all locations referenced by this block.
 * @return {!Array.<string>} List of locations.
 */
Blockly.Block.prototype.getLocs = function() {
  var vars = [];
  for (var i = 0, input; input = this.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof Blockly.FieldLocation) {
        vars.push(field.getValue());
      }
    }
  }
 ...
 * @param {string} newName Renamed variable.
 */
Blockly.Block.prototype.renameLoc = function(oldName, newName) {
  for (var i = 0, input; input = this.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof Blockly.FieldLocation &&
          Blockly.Names.equals(oldName, field.getValue())) {
